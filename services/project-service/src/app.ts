...
app.use((_req, _res, next) => {
  ...
  _res.on('finish', () => {
    ...
    if (ms > 2000) logger.warn('Slow request', { method: _req.method, url: _req.originalUrl, durationMs: ms });
  });
  next();
});
...
app.use('/api/projects/:projectId/matches',     apiRateLimiter, matchesRouter);
app.use('/api/contracts',                       apiRateLimiter, contractsRouter);
